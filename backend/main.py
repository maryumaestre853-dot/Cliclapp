...
from models import Base, Usuario, Bicicleta, DocCicla, Ruta, PuntoServicio, Recorrido, AppRoute
...
class AppRouteCreate(BaseModel):
...
def pausar_recorrido(id: int, db: Session = Depends(get_db)):
    ...
    db.commit()
    return rec
# ─── APP ROUTES / HISTORIAL DE RUTAS ─────────────────────────
@app.get("/app-rutas")
def get_app_routes(db: Session = Depends(get_db)):
    return db.query(AppRoute).order_by(AppRoute.date.desc()).all()

@app.post("/app-rutas")
def crear_app_route(route: AppRouteCreate, db: Session = Depends(get_db)):
    datos = route.dict()
    if datos["date"] is None:
        datos["date"] = datetime.utcnow()
    nueva = AppRoute(**datos)
    db.add(nueva)
    db.commit()
    db.refresh(nueva)
    return nueva

@app.delete("/app-rutas/{id}")
def eliminar_app_route(id: int, db: Session = Depends(get_db)):
    ruta = db.query(AppRoute).filter(AppRoute.id == id).first()
    if not ruta:
        raise HTTPException(status_code=404, detail="Ruta no encontrada")
    db.delete(ruta)
    db.commit()
